...
if FEMord==1
nte=numel(te2p)/4;
mex_id_ = 'secondaryEfieldc(i double[xx], i int, i int[xx], i int[xx], i int, i double[xx], i double[x], io double[xx], i double)';
[Eout] = FEM(mex_id_, robs, nobs, te2te, te2p, nte, p, xval, Eout, iprecEp, 3, nobs, 4, nte, 4, nte, 3, np, np, 3, nobs);
elseif FEMord==2
nte=numel(te2p)/10;
mex_id_ = 'secondaryEfield2ndc(i double[xx], i int, i int[xx], i int[xx], i int, i double[xx], i double[x], io double[xx], i double)';
[Eout] = FEM(mex_id_, robs, nobs, te2te, te2p, nte, p, xval, Eout, iprecEp, 3, nobs, 4, nte, 10, nte, 3, np, np, 3, nobs);
elseif FEMord==3
nte=numel(te2p)/20;
mex_id_ = 'secondaryEfield3rdc(i double[xx], i int, i int[xx], i int[xx], i int, i double[xx], i double[x], io double[xx], i double)';

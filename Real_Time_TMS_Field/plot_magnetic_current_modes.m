function [current_axis] = plot_magnetic_current_modes(current_axis,msh_file,msh_file_read_fcn,mapping_surface,m2m_dir,...
    ...
    if randomized
        prc = 0.2;
        rand_id = randperm(size(magnetic_currents,2),ceil(prc*size(magnetic_currents,2)));
        rs = rs(:,rand_id);
        magnetic_currents = magnetic_currents(:,rand_id);
    end
    ...
    quiver3(current_axis,rs(1,:),rs(2,:),rs(3,:),magnetic_currents(1,:),magnetic_currents(2,:),magnetic_currents(3,:),5,'b','AutoScale','on','AutoScaleFactor',2)
    light(current_axis)
